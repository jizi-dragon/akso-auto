...
| 已完成 | status_Done | — | — | 关闭（action_Close）→ 已关闭 |
...
指令 8.2.2 — 填写工作流基本信息
  【目标】配置工作流名称、Code、触发条件
  【前置条件】已在工作流编辑页
  【关键定位】
    - 名称：textbox（名称输入框）
    - Code：textbox "请输入Code"
    - 触发条件：一般在右侧属性面板或顶部配置区
  【操作步骤】
    1. 填写名称（中文）：如 "变更审批流程"
    2. 填写 Code：workflow_ + 英文，如 workflow_ChangeApproval
    3. 设置触发条件：
       - 选择触发时机：创建时触发 / 状态变更时触发 / 字段变更时触发
       - 常见：状态变更时触发，指定某个动作（如"提交"）
  【验证方法】基本信息区域显示已填写的名称和 Code；触发条件下拉已选中
```
...
□ 9. 工作流：是否已创建并配置参与者/任务/判断/通知节点？
□ 10. 工作流：节点连线是否完整？判断节点分支是否齐全？
□ 11. 工作流：是否已保存并激活？
□ 12. 菜单：是否已创建？关联对象和布局是否正确？
□ 13. 权限集：是否已创建？对象级权限是否配置？
□ 14. 字段安全：敏感字段是否设置了只读/隐藏？
□ 15. 菜单与权限：菜单是否关联了权限集？
□ 16. 验证测试：业务端是否可通过菜单新建记录并走通流程？
```
