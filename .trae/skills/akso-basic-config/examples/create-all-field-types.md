...
```bash
node scripts/create-field.js --objectName "测试对象A" --name "文本字段" --code "text_field" --dataType "文本"
```

支持批量 JSON 文件传入：

```bash
node scripts/create-field.js --batch fields-batch.json --objectName "测试对象A"
```
