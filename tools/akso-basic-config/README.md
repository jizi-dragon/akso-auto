...
> **环境信息**：存储于外部 `AksoGMP_配置环境清单.xlsx`，本工具不含密码/账号

> 📂 **文件结构**：
> ```
> akso-basic-config/
> ├── README.md         ← 你在这里（核心指令集）
> ├── examples/         ← 实战示例（login / create-object / create-fields / create-option-set）
> ├── lib/              ← 可复用脚本（登录 / 类型选择 / 选项集绑定 / 对象绑定 / 父对象绑定 / 助手函数）
> └── index.js          ← 工具入口
> ```
> `lib/*.js` 为 Node.js 可执行模块，通过 `require('../../shared/browser-manager')` 统一管理浏览器生命周期。可直接 `node lib/<name>.js` 运行，也可在编排脚本中 `require` 导入使用。参考 `examples/*.md` 中的完整操作流程。
...
> **文件位置**：与工具同项目的 `d:/akso/akso_claw/AksoGMP_配置环境清单.xlsx`。
...
> **规则 1.1.0 — 环境信息分离存储**：  
> 登录地址、账号、密码等敏感信息**不在本工具中硬编码**，统一存储在外部 Excel 文件：  
...
指令 1.2.1 — 打开浏览器并导航
  操作：使用 `shared/browser-manager.js` 的 `launchBrowser()` 启动浏览器，然后 `login(page, {...})` 完成登录
        也可直接运行 `node lib/login.js --baseUrl <url> --username <user> --password <pass>`
...
指令 1.2.6 — ⚡ 一键登录（推荐，3 秒内完成）
  ...
  方式一（直接运行）：
    node lib/login.js --baseUrl https://xxx.aksoegmp.com --username user --password pass
...
技巧 B.12 — ⚡ 一键登录：使用 `node lib/login.js` 或 `require` 引入
  ❌ 慢（旧方式）：playwright-cli open → eval → run-code（3 次 CLI 调用）
  ✅ 快（npm 包）：node lib/login.js --baseUrl --username --password（1 次命令）
...
> 所有原子模块存放在 `lib/` 目录，为 Node.js 可执行模块。  
> 通过 `shared/browser-manager.js` 统一管理浏览器生命周期。  
> 每个模块支持两种使用方式：`node lib/<name>.js <args>` 直接运行，或在编排脚本中 `require` 导入。

| 文件名 | 功能 | 导出函数 | 独立运行方式 |
|--------|------|----------|-------------|
| [`login.js`](lib/login.js) | 登录（含隐私弹窗） | `login(page, opts)` | `node login.js --baseUrl --username --password` |
| [`dismiss-popup.js`](lib/dismiss-popup.js) | 弹窗清除 | `dismissAllPopups(page)` | —（辅助模块） |
| [`exact-search-click.js`](lib/exact-search-click.js) | 精确搜索点击 | `exactSearchClick(page, opts)` | `node exact-search-click.js --keyword --exactName --listUrl` |
| [`create-option-set.js`](lib/create-option-set.js) | 创建选项集 | `createOptionSet(page, opts)` | `node create-option-set.js --name --code --options` |
| [`create-object.js`](lib/create-object.js) | 创建对象 | `createObject(page, opts)` | `node create-object.js --name --code --lifecycle` |
| [`create-field.js`](lib/create-field.js) | 创建单个字段 | `createField(page, opts)` | `node create-field.js --name --code --dataType --objectName ...` |
| [`save-and-verify.js`](lib/save-and-verify.js) | 保存+验证 | `saveAndVerify(page, opts)` | `node save-and-verify.js --expectedUrlPattern ...` |
| [`lifecycle.js`](lib/lifecycle.js) | 生命周期统一入口（含 4 子模块） | `createStatus`, `createEntryCondition`, `createEntryReaction`, `createUserAction`, `selectAntOption` | —（编排模块） |
| [`lifecycle-create-state.js`](lib/lifecycle-create-state.js) | 创建生命周期状态 | `createStatus(page, opts)` | `node lifecycle-create-state.js --lifecycleId --name --code` |
| [`lifecycle-entry-condition.js`](lib/lifecycle-entry-condition.js) | 创建进入条件 | `createEntryCondition(page, opts)` | `node lifecycle-entry-condition.js --lifecycleId --statusId --condField --errorMsg` |
| [`lifecycle-entry-reaction.js`](lib/lifecycle-entry-reaction.js) | 创建进入动作 | `createEntryReaction(page, opts)` | `node lifecycle-entry-reaction.js --lifecycleId --statusId --actionType` |
| [`lifecycle-entry-action.js`](lib/lifecycle-entry-action.js) | 创建用户动作 | `createUserAction(page, opts)` | `node lifecycle-entry-action.js --lifecycleId --statusId --actionName --code` |
| [`code-rule.js`](lib/code-rule.js) | 创建编号规则 | `createCodeRule(page, opts)` | `node code-rule.js --name --code --objectName --rules` |

### 辅助模块

| 文件名 | 功能 | 导出函数 |
|--------|------|----------|
| [`helpers.js`](lib/helpers.js) | 通用辅助函数集 | `verifyFieldSaved`, `safeFill`, `selectFromAntSelect`, 等 8 个 |
| [`field-type-select.js`](lib/field-type-select.js) | 字段类型选择 | `selectFieldType(page, typeName)` |
| [`object-bind.js`](lib/object-bind.js) | 对象字段绑定 | `bindObject(page, objectName)` |
| [`option-set-bind.js`](lib/option-set-bind.js) | 选项集字段绑定 | `bindOptionSet(page, optionSetName)` |
| [`parent-object-bind.js`](lib/parent-object-bind.js) | 父对象字段绑定 | `bindParentObject(page, objectName)` |
...
```bash
# 登录（直接运行）
node tools/akso-basic-config/lib/login.js \
  --baseUrl https://standard-val.aksoegmp.com \
  --username liyulong \
  --password 88888888

# 创建对象（直接运行）
node tools/akso-basic-config/lib/create-object.js \
  --name "测试对象" --code "test_obj" --lifecycle

# 创建生命周期状态（直接运行）
node tools/akso-basic-config/lib/lifecycle-create-state.js \
...
> - 🔧 **所有 lib/*.js 重构**：从 CLI `async(page)=>{}` 模式改为 Node.js 模块，支持 `require` 引入和直接 `node` 执行
...
> - 📝 **同步修复**：B.14 和 lib/field-type-select.js 中的 exact:true 错误建议
>
> **往期更新（v4.1 — 性能与容错）**：
